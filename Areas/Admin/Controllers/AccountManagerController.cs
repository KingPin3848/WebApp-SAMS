...
namespace SAMS.Areas.Admin.Controllers
{
	...
	internal class AccountManagerController : Controller
	{
		...
		internal class indexClass
		{
			...
			public required string flname { get; set; }
		...
		/* GET: Admin/AccountManager/Index
			* Index Function Description:
			* En-lists all users registered within the application, 
			* except the developer accounts
			* Details passed at the end as an output: Selected UserManager Fields 
			* through IActionResult to the View - as seen in indexClass
			* Details shown: SchoolId, Role, Email, and Activation Code
			* 
			* CHANGES NEEDED:
			* 1. New set of event ids and code-identifiers needed.
			* 2. ApplicationUser now must have key identifying details such as full name.
			* 
			* ADDITIONS NEEDED:
			* 1. Authorized to only Admins after two-factor authentication. Add 2FA - only using the Authenticator app
			* OR request another Google sign-in along with Google's own 2FA.
		*/
		[HttpGet]
		[RequireHttps]
		...
		public IActionResult Index()
		{
			...
			{
				condensedList.Add(new indexClass
				{
					dbid = user.Id,
		...
		/* GET: Admin/AccountManager/Details/id
		 * Details Function Description:
		 * En-lists selected details of the user selected by the admin.
		 * Details passed at the end as an output: Selected UserManager Fields
		 * through IActionResult to the View - as seen in the detailsClass
		 * Details shown: SchoolId, Role, Email, Activation Code
		 * 
		 * CHANGES NEEDED:
		 * 1. New set of event ids and code-identifiers needed.
		 * 2. Code refactor
		...
		public async Task<IActionResult> Details(string id)
		{
			...
			var detailUser = await userManager.FindByIdAsync(id).ConfigureAwait(true);
